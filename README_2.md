...
(github.com/sad192/LISN-Infrared-Image-SR) does implement the LISB structure — a double
channel split (`torch.split`), a pointwise+depthwise residual block equivalent to the paper's
RDB (named `SRB`), and contrast-aware channel attention (`CCALayer`) — but it replaces the
paper's core novelty, the zero-parameter shift block (SBB), with `SwinT` windowed
self-attention (`num_heads=8`). `models/network_hybrid.py` also imports `models/fusion.py`,
which is not present in the repository, so the code does not import as released. No
pretrained weights are available (open issue #4, 11 July 2025).
